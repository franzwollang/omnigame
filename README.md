...
- **Connect 4 (Pop Out)**
- **Gomoku (5‑in‑a‑row)**
- **Capture / Flip Demo** (Reversi-like flips; ends via n-in-a-row, not Othello scoring)
...
- FP runtime: Effect (Effect.ts) — foothold in core (`src/engine/rng.ts`); GameKernel migration next
- Turn scaffolding: hand-rolled phase helper (`turnMachine.ts`); XState not wired
- Rendering: Three.js (board/camera), ResizeObserver
- Editor: react-simple-code-editor + Prism.js
- Tests: Vitest (transcript / unit)
...
The core follows pure functional principles with reducers (`State -> Event -> State`). Effect is adopted for FP primitives at the core boundary (seeded RNG foothold today; stepping/legal-actions move behind `GameKernel` next). Turn phases use a small hand-rolled scaffold until a richer machine is justified.
...
- **Topology**: rectangular grid only (`grid.topology = "rectangle"`); edge wrap deferred
...
- **Placement**:
  ...
  - overflow: `reject` | `pop_out_bottom` (top pop-out deferred)
- **Effects**: optional capture toggles (Capture / Flip demo)
- **Objectives**: n-in-a-row win detection with configurable adjacency + length
- **Determinism**: `rng.seed` + Effect RNG helpers (not yet consumed by play loop)
