...
export const examplePresets: Record<string, ExamplePreset> = {
	...
	"hidden-simultaneous-jump-race": definePreset({
		id: "hidden-simultaneous-jump-race",
		...
		config: {
			...
			placements: [],
			initial: [
				// X(2,0) commits jump mid O(1,1) → (0,2); O runner (4,4) quiet steps.
				{ row: 2, col: 0, player: "X", visibility: "public" },
				{ row: 1, col: 1, player: "O", visibility: "public" },
				{ row: 4, col: 4, player: "O", visibility: "public" }
			]
	...
	"go-lite-l-nakade": definePreset({
		id: "go-lite-l-nakade",
		name: "Go Lite L-Nakade",
		tags: [
			"liberties",
			"territory",
			"area-control",
		...
		description:
			"Go Lite with bent-3 (L) nakade removal at scoring (objective.lNakadeDeath). Seeded interior X shell with one true eye + closed L corridor (Benson-alive; T1 nakadeDeath misses) plus living edge O: without the flag (or with bensonLife / nakadeDeath) double-pass awards X; with lNakadeDeath X is cleared → O wins. Unlocks L-shape big-eye death beyond T1 straight nakade / Benson / chase family.",
		config: {
			metadata: { name: "Go Lite L-Nakade", version: 1 },
			grid: { width: 7, height: 7, topology: "rectangle", wrap: false },
			turn: { mode: "turn" },
			rng: { seed: 42 },
			input: { mode: "cell" },
			...
			observation: { mode: "full" },
			objective: { mode: "area_control", lNakadeDeath: true },
			tokens: [
				{
					id: "stone-x",
					label: "●",
			...
			initial: (() => {
				...
				// Interior X shell (rows/cols 1–5): true eye at (2,2); closed
				// bent-3 L corridor at (3,4)/(4,4)/(4,3). One true eye
				// (dead under deadStones) but Benson-alive via eye + corridor;
				// T1 nakadeDeath misses (not colinear).
				const xCells: Array<[number, number]> = [
					[1, 1],
					[1, 2],
					[1, 3],
				...
				// Living edge O ring — raw X still leads on area; after L-nakade
				// clears X, O wins.
				for (let row = 0; row < 7; row++) {
					for (let col = 0; col < 7; col++) {
						if (row === 0 || row === 6 || col === 0 || col === 6) {
	...
	"go-lite-twisted-nakade": definePreset({
		id: "go-lite-twisted-nakade",
		name: "Go Lite Twisted-Nakade",
		tags: [
			"liberties",
			"territory",
			"area-control",
		...
		description:
			"Go Lite with twisted-4 (Z/S) nakade removal at scoring (objective.twistedNakadeDeath). Seeded interior X shell with one true eye + closed skew corridor (Benson-alive; T1/L/square/pyramid nakadeDeath miss; chase family miss) plus living edge O: without the flag (or with bensonLife / nakadeDeath / lNakadeDeath / squareNakadeDeath / pyramidNakadeDeath) double-pass awards X; with twistedNakadeDeath X is cleared → O wins. Unlocks Z/S-shape bulky big-eye death beyond T1/L/square/pyramid nakade / Benson / chase family.",
		config: {
			metadata: { name: "Go Lite Twisted-Nakade", version: 1 },
			grid: { width: 7, height: 7, topology: "rectangle", wrap: false },
			turn: { mode: "turn" },
			rng: { seed: 42 },
			input: { mode: "cell" },
			...
			observation: { mode: "full" },
			objective: { mode: "area_control", twistedNakadeDeath: true },
			tokens: [
				{
					id: "stone-x",
					label: "●",
			...
			initial: (() => {
				...
				// Interior X shell (rows/cols 1–5): true eye at (2,2); closed
				// twisted-4 Z/S corridor at (3,3)/(3,4)/(4,2)/(4,3) (skew;
				// vital = canonical deg-2 (3,3)). One true eye (dead under
				// deadStones) but Benson-alive via eye + skew; T1/L/square/
				// pyramid miss.
				const xCells: Array<[number, number]> = [
					[1, 1],
					[1, 2],
	...
	"go-lite-l4-nakade": definePreset({
		id: "go-lite-l4-nakade",
		name: "Go Lite L4-Nakade",
		tags: [
			"liberties",
			"territory",
			"area-control",
		...
		description:
			"Go Lite with L4 (tetromino L/J) nakade removal at scoring (objective.l4NakadeDeath). Seeded interior X shell with one true eye + closed L corridor (Benson-alive; T1/L/square/pyramid/twisted nakadeDeath miss; chase family miss) plus living edge O: without the flag (or with bensonLife / nakadeDeath / lNakadeDeath / squareNakadeDeath / pyramidNakadeDeath / twistedNakadeDeath) double-pass awards X; with l4NakadeDeath X is cleared → O wins. Unlocks L/J-shape bulky big-eye death beyond T1/L/square/pyramid/twisted nakade / Benson / chase family.",
		config: {
			metadata: { name: "Go Lite L4-Nakade", version: 1 },
			grid: { width: 7, height: 7, topology: "rectangle", wrap: false },
			turn: { mode: "turn" },
			rng: { seed: 42 },
			input: { mode: "cell" },
			...
			observation: { mode: "full" },
			objective: { mode: "area_control", l4NakadeDeath: true },
			tokens: [
				{
					id: "stone-x",
					label: "●",
			...
			initial: (() => {
				...
				// Interior X shell (rows/cols 1–5): true eye at (2,2); closed
				// L4 corridor at (3,4)/(4,2)/(4,3)/(4,4) (same-side bbox holes;
				// vital = canonical deg-2 (4,3)). One true eye (dead under
				// deadStones) but Benson-alive via eye + L; T1/L/square/
				// pyramid/twisted miss.
				const xCells: Array<[number, number]> = [
					[1, 1],
					[1, 2],
					[1, 3],
	...
	"go-lite-bulky5-nakade": definePreset({
		id: "go-lite-bulky5-nakade",
		name: "Go Lite Bulky-5-Nakade",
		tags: [
			"liberties",
			"territory",
			"area-control",
		...
		description:
			"Go Lite with bulky-5 (P-pentomino) nakade removal at scoring (objective.bulky5NakadeDeath). Seeded interior X shell on 7×7 with one true eye + closed P corridor (Benson-alive; T1/L/square/pyramid/twisted/L4/straight-4 nakadeDeath miss; chase family miss) plus living edge O: without the flag (or with bensonLife / prior nakade flags) double-pass awards X; with bulky5NakadeDeath X is cleared → O wins. Unlocks the first pentomino big-eye death beyond tetromino nakade / Benson / chase family.",
		config: {
			metadata: { name: "Go Lite Bulky-5-Nakade", version: 1 },
			grid: { width: 7, height: 7, topology: "rectangle", wrap: false },
			turn: { mode: "turn" },
			rng: { seed: 42 },
			input: { mode: "cell" },
			...
			observation: { mode: "full" },
			objective: { mode: "area_control", bulky5NakadeDeath: true },
			tokens: [
				{
					id: "stone-x",
					label: "●",
			...
			initial: (() => {
				...
				// Interior X shell (rows/cols 1–5): true eye at (4,4); closed
				// P-pentomino corridor at (2,2)/(2,3)/(2,4)/(3,2)/(3,3)
				// (2×3 minus corner (3,4); vital = unique deg-3 (2,3)). One
				// true eye (dead under deadStones) but Benson-alive via eye +
				// P; tetromino nakade tables miss (region length 5).
				const empties = new Set([
					"4,4",
					"2,2",
					"2,3",
	...
	"go-lite-plus-nakade": definePreset({
		id: "go-lite-plus-nakade",
		name: "Go Lite Plus-Nakade",
		tags: [
			"liberties",
			"territory",
			"area-control",
		...
		description:
			"Go Lite with plus / X-pentomino nakade removal at scoring (objective.plusNakadeDeath). Seeded interior X shell on 9×9 with one true eye + closed plus corridor (Benson-alive; T1/L/square/pyramid/twisted/L4/straight-4/bulky-5 nakadeDeath miss; chase family miss) plus living edge O: without the flag (or with bensonLife / prior nakade flags) double-pass awards X; with plusNakadeDeath X is cleared → O wins. Unlocks the plus big-eye death beyond bulky-5 / tetromino nakade / Benson / chase family (needs ≥9×9 for plus + disjoint eye).",
		config: {
			metadata: { name: "Go Lite Plus-Nakade", version: 1 },
			...
			observation: { mode: "full" },
			objective: { mode: "area_control", plusNakadeDeath: true },
			tokens: [
				{
					id: "stone-x",
					label: "●",
			...
			initial: (() => {
				...
				// plus / X-pentomino at (2,3)/(3,2)/(3,3)/(3,4)/(4,3)
				// (3×3 minus four corners; vital = center deg-4 (3,3)). One
				// true eye (dead under deadStones) but Benson-alive via eye +
				// plus; bulky-5 / tetromino tables miss (plus ≠ P / tetromino).
				const empties = new Set([
					"6,6",
					"2,3",
					"3,2",
	...
	"go-lite-rabbity6-nakade": definePreset({
		id: "go-lite-rabbity6-nakade",
		name: "Go Lite Rabbity-Six-Nakade",
		tags: [
			"liberties",
			"territory",
			"area-control",
		...
		description:
			"Go Lite with rabbity-six (filled 2×3 hexomino) nakade removal at scoring (objective.rabbitySixNakadeDeath). Seeded interior X shell on 9×9 with one true eye + closed 2×3 corridor (Benson-alive; T1/L/square/pyramid/twisted/L4/straight-4/bulky-5/plus/V nakadeDeath miss; chase family miss) plus living edge O: without the flag (or with bensonLife / prior nakade flags) double-pass awards X; with rabbitySixNakadeDeath X is cleared → O wins. Unlocks the first hexomino big-eye death beyond V / plus / bulky-5 / tetromino nakade / Benson / chase family (needs ≥9×9 for 2×3 + disjoint eye).",
		config: {
			metadata: { name: "Go Lite Rabbity-Six-Nakade", version: 1 },
			grid: { width: 9, height: 9, topology: "rectangle", wrap: false },
			turn: { mode: "turn" },
			rng: { seed: 42 },
			input: { mode: "cell" },
			...
			observation: { mode: "full" },
			objective: { mode: "area_control", rabbitySixNakadeDeath: true },
			tokens: [
				{
					id: "stone-x",
					label: "●",
			...
			initial: (() => {
				...
				// Interior X shell (rows/cols 1–7): true eye at (6,6); closed
				// rabbity-six / filled 2×3 at (2,2)/(2,3)/(2,4)/(3,2)/(3,3)/(3,4)
				// (vital = lex-first deg-3 (2,3)). One true eye (dead under
				// deadStones) but Benson-alive via eye + hexomino; V / plus /
				// bulky-5 / tetromino tables miss (region length 6).
				const empties = new Set([
					"6,6",
					"2,2",
					"2,3",
