...
/**
 * Twisted-4 (Z/S skew) nakade: exactly four empties in a 2×3 / 3×2 bbox
...
/**
 * Straight-4 (I-tetromino) nakade: exactly four empties in a filled 1×4 /
 * 4×1 bbox (colinear consecutive); vital = canonical 2nd cell on the sorted
 * line (one of the two Go vitals). Distinct from T1 (length 3) / L4 / twisted /
 * pyramid / square so the last free tetromino stays contrastable (M91).
 */
export function isStraight4NakadeVulnerableRegion(
	regionCells: Position[]
): { vital: Position } | null {
	if (regionCells.length !== 4) return null;
	if (isSquareNakadeVulnerableRegion(regionCells)) return null;
	if (isPyramidNakadeVulnerableRegion(regionCells)) return null;
	if (isTwistedNakadeVulnerableRegion(regionCells)) return null;
	if (isL4NakadeVulnerableRegion(regionCells)) return null;
	let minR = Infinity;
	let maxR = -Infinity;
	let minC = Infinity;
	let maxC = -Infinity;
	...
	const h = maxR - minR;
	const w = maxC - minC;
	if (!((h === 0 && w === 3) || (h === 3 && w === 0))) return null;
	// All four cells must fill the 1×4 / 4×1 bbox (no gaps).
	if (h === 0) {
		const cols = new Set(regionCells.map((p) => p.col));
		if (cols.size !== 4) return null;
		for (let c = minC; c <= maxC; c++) {
			if (!cols.has(c)) return null;
		}
	} else {
		const rows = new Set(regionCells.map((p) => p.row));
		if (rows.size !== 4) return null;
		for (let r = minR; r <= maxR; r++) {
			if (!rows.has(r)) return null;
		}
	}
	const sorted = [...regionCells].sort(
		(p, q) => p.row - q.row || p.col - q.col
	);
	return { vital: sorted[1]! };
}
...
/**
 * Plus / X-pentomino nakade: exactly five empties in a 3×3 bbox missing the
 * four corners (orthogonal cross); vital = unique degree-4 center. Distinct
 * from bulky-5 (P) and tetromino nakade tables so the plus big-eye stays
 * contrastable (M93).
 */
export function isPlusNakadeVulnerableRegion(
	regionCells: Position[]
): { vital: Position } | null {
	if (regionCells.length !== 5) return null;
	let minR = Infinity;
	...
	if (degrees.filter((d) => d === 4).length !== 1) return null;
	if (degrees.filter((d) => d === 1).length !== 4) return null;
	const centerIdx = degrees.findIndex((d) => d === 4);
	return { vital: regionCells[centerIdx]! };
}

/**
 * V-pentomino nakade: exactly five empties in a 3×3 bbox whose four holes form
 * a filled 2×2 block in one corner (two full adjacent sides); vital = unique
 * occupied bbox-corner whose two in-region neighbors are not colinear (the V
 * vertex). Distinct from plus (four corner holes) and bulky-5 (2×3 / 3×2) so
 * the V big-eye stays contrastable (M94).
 */
export function isVNakadeVulnerableRegion(
	regionCells: Position[]
): { vital: Position } | null {
	if (regionCells.length !== 5) return null;
	if (isBulky5NakadeVulnerableRegion(regionCells)) return null;
	if (isPlusNakadeVulnerableRegion(regionCells)) return null;
	let minR = Infinity;
	let maxR = -Infinity;
	let minC = Infinity;
	let maxC = -Infinity;
	...
	for (const m of missing) {
		if (m.row < holeMinR) holeMinR = m.row;
		if (m.row > holeMaxR) holeMaxR = m.row;
		if (m.col < holeMinC) holeMinC = m.col;
		if (m.col > holeMaxC) holeMaxC = m.col;
	}
	if (holeMaxR - holeMinR !== 1 || holeMaxC - holeMinC !== 1) return null;
	const holeKeys = new Set(missing.map((m) => `${m.row},${m.col}`));
	for (let r = holeMinR; r <= holeMaxR; r++) {
		for (let c = holeMinC; c <= holeMaxC; c++) {
			if (!holeKeys.has(`${r},${c}`)) return null;
		}
	}
	...
	for (const corner of corners) {
		if (!keys.has(`${corner.row},${corner.col}`)) continue;
		const neighbors = regionCells.filter((q) => ortho(corner, q));
 ...
 * First 6-cell nakade class; distinct from pentomino / tetromino tables (M95).
 */
export function isRabbitySixNakadeVulnerableRegion(
	regionCells: Position[]
): { vital: Position } | null {
	if (regionCells.length !== 6) return null;
	let minR = Infinity;
	let maxR = -Infinity;
	let minC = Infinity;
	let maxC = -Infinity;
	...
	if (degrees.filter((d) => d === 3).length !== 2) return null;
	if (degrees.filter((d) => d === 2).length !== 4) return null;
	if (degrees.some((d) => d !== 2 && d !== 3)) return null;
	const vitals = degrees
		.map((d, i) => (d === 3 ? regionCells[i]! : null))
		.filter((p): p is Position => p !== null)
		.sort((p, q) => p.row - q.row || p.col - q.col);
	return { vital: vitals[0]! };
}
...
/**
 * Interior groups that border a twisted-4 (Z/S) nakade big-eye and would have
 * fewer than 2 true eyes after an opponent vital fill are dead at scoring
 * (M89). Same exemptions as T1/L/square/pyramid nakade (edge + seki).
 * Contrastable with pyramid/square/T1/L and chase family.
 */
export function findTwistedNakadeDeadCells(
	grid: Grid,
	wrap: boolean = false,
	topology: GridTopology = "rectangle",
	graph?: GraphTopologyData
	...
	for (const color of ["X", "O"] as const) {
		...
		for (const region of regions) {
			const nakade = isTwistedNakadeVulnerableRegion(region.cells);
			if (!nakade) continue;
			const sim = simulateLibertyPlace(
				grid,
				nakade.vital,
...
/** Clear twisted-nakade-dead stones from a grid copy (M89). */
export function removeTwistedNakadeDeadStones(
	grid: Grid,
	wrap: boolean = false,
	topology: GridTopology = "rectangle",
	graph?: GraphTopologyData
...
/**
 * Interior groups that border a straight-4 (I-tetromino) nakade big-eye and
 * would have fewer than 2 true eyes after an opponent vital fill are dead at
 * scoring (M91). Same exemptions as T1/L/square/pyramid/twisted/L4 nakade
 * (edge + seki). Contrastable with L4/twisted/pyramid/square/T1/L and chase.
 */
export function findStraight4NakadeDeadCells(
	grid: Grid,
	wrap: boolean = false,
	topology: GridTopology = "rectangle",
	graph?: GraphTopologyData
	...
	for (const color of ["X", "O"] as const) {
		...
		for (const region of regions) {
			const nakade = isStraight4NakadeVulnerableRegion(region.cells);
			if (!nakade) continue;
			const sim = simulateLibertyPlace(
				grid,
				nakade.vital,
...
/** Clear straight-4-nakade-dead stones from a grid copy (M91). */
export function removeStraight4NakadeDeadStones(
	grid: Grid,
	wrap: boolean = false,
	topology: GridTopology = "rectangle",
	graph?: GraphTopologyData
...
/**
 * Interior groups that border a plus / X-pentomino nakade big-eye and would
 * have fewer than 2 true eyes after an opponent vital fill are dead at
 * scoring (M93). Same exemptions as bulky-5 / tetromino nakade (edge + seki).
 * Contrastable with bulky-5 (P) and tetromino nakade / chase family.
 */
export function findPlusNakadeDeadCells(
	grid: Grid,
	wrap: boolean = false,
	topology: GridTopology = "rectangle",
	graph?: GraphTopologyData
	...
	for (const color of ["X", "O"] as const) {
		...
		for (const region of regions) {
			const nakade = isPlusNakadeVulnerableRegion(region.cells);
			if (!nakade) continue;
			const sim = simulateLibertyPlace(
				grid,
				nakade.vital,
...
/** Clear plus-nakade-dead stones from a grid copy (M93). */
export function removePlusNakadeDeadStones(
	grid: Grid,
	wrap: boolean = false,
	topology: GridTopology = "rectangle",
	graph?: GraphTopologyData
): Grid {
	const dead = findPlusNakadeDeadCells(grid, wrap, topology, graph);
	if (dead.length === 0) return grid;
	let cells = grid.cells;
	for (const p of dead) {
		cells = setCell({ ...grid, cells }, p, null);
...
/**
 * Interior groups that border a V-pentomino nakade big-eye and would have
 * fewer than 2 true eyes after an opponent vital fill are dead at scoring
 * (M94). Same exemptions as plus / bulky-5 / tetromino nakade (edge + seki).
 * Contrastable with plus (X) / bulky-5 (P) and tetromino nakade / chase family.
 */
export function findVNakadeDeadCells(
	grid: Grid,
	wrap: boolean = false,
	topology: GridTopology = "rectangle",
	graph?: GraphTopologyData
	...
	for (const color of ["X", "O"] as const) {
		...
		for (const region of regions) {
			const nakade = isVNakadeVulnerableRegion(region.cells);
			if (!nakade) continue;
			const sim = simulateLibertyPlace(
				grid,
				nakade.vital,
...
/** Clear V-nakade-dead stones from a grid copy (M94). */
export function removeVNakadeDeadStones(
	grid: Grid,
	wrap: boolean = false,
	topology: GridTopology = "rectangle",
	graph?: GraphTopologyData
): Grid {
	const dead = findVNakadeDeadCells(grid, wrap, topology, graph);
	if (dead.length === 0) return grid;
	let cells = grid.cells;
	for (const p of dead) {
		cells = setCell({ ...grid, cells }, p, null);
...
/**
 * Interior groups that border a rabbity-six (filled 2×3 hexomino) nakade
 * big-eye and would have fewer than 2 true eyes after an opponent vital fill
 * are dead at scoring (M95). Same exemptions as V / plus / bulky-5 nakade
 * (edge + seki). First 6-cell class; contrastable with pentomino / tetromino
 * nakade and chase family.
 */
export function findRabbitySixNakadeDeadCells(
	grid: Grid,
	wrap: boolean = false,
	topology: GridTopology = "rectangle",
	graph?: GraphTopologyData
	...
	for (const color of ["X", "O"] as const) {
		...
		for (const region of regions) {
			const nakade = isRabbitySixNakadeVulnerableRegion(region.cells);
			if (!nakade) continue;
			const sim = simulateLibertyPlace(
				grid,
				nakade.vital,
...
/** Clear rabbity-six-nakade-dead stones from a grid copy (M95). */
export function removeRabbitySixNakadeDeadStones(
	grid: Grid,
	wrap: boolean = false,
	topology: GridTopology = "rectangle",
	graph?: GraphTopologyData
): Grid {
	const dead = findRabbitySixNakadeDeadCells(grid, wrap, topology, graph);
	if (dead.length === 0) return grid;
	let cells = grid.cells;
	for (const p of dead) {
		cells = setCell({ ...grid, cells }, p, null);
 ...
 * big-eyes runs after optional straight4NakadeDeath and before
 * plusNakadeDeath / netDeath (M92). When `plusNakadeDeath` is true, the same
 * path for plus / X-pentomino big-eyes runs after optional bulky5NakadeDeath
 * and before vNakadeDeath / netDeath (M93). When `vNakadeDeath` is true, the
...
export function areaOutcome(
	...
	if (plusNakadeDeath) {
		scored = removePlusNakadeDeadStones(scored, wrap, topology, graph);
	}
	if (vNakadeDeath) {
		scored = removeVNakadeDeadStones(scored, wrap, topology, graph);
	}
