...
import {
	...
	findVNakadeDeadCells,
	findRabbitySixNakadeDeadCells,
	findUNakadeDeadCells,
	findNetDeadCells,
	...
	isVNakadeVulnerableRegion,
	isRabbitySixNakadeVulnerableRegion,
	isUNakadeVulnerableRegion,
	isNetDeadGroup,
	isLooseNetDeadGroup,
	...
	removeVNakadeDeadStones,
	removeRabbitySixNakadeDeadStones,
	removeUNakadeDeadStones,
	removeNetDeadStones,
...
describe("Go Lite (liberties + area_control)", () => {
	...
	it("validates and compiles the go-lite-v-nakade preset", () => {
		const cfg = examplePresets["go-lite-v-nakade"].config;
		expect(validateConfig(cfg).ok).toBe(true);
		const { kernel, gameConfig } = compileConfig(cfg);
		expect(gameConfig.objectiveMode).toBe("area_control");
		expect(gameConfig.vNakadeDeath).toBe(true);
		expect(gameConfig.captureMode).toBe("liberties");
		const state = kernel.initialState(cfg.rng.seed);
		expect(kernel.legalActions(state, 0).some((a) => a.type === "pass")).toBe(
			true
	...
	it("rejects objective.vNakadeDeath outside area_control", () => {
		const bad = structuredClone(examplePresets["tic-tac-toe"].config) as {
			objective: { mode: string; vNakadeDeath?: boolean };
		};
		bad.objective = { mode: "n_in_a_row", vNakadeDeath: true };
		expect(validateConfig(bad as never).ok).toBe(false);
	});

	it("findVNakadeDeadCells: V corridor clears Benson-alive one-eyed shell; prior nakade + chase miss", () => {
		const cfg = examplePresets["go-lite-v-nakade"].config;
		const { kernel } = compileConfig(cfg);
		const state = kernel.initialState(cfg.rng.seed);
		const g = state.grid;
		...
		expect(
			findPlusNakadeDeadCells(g).every((p) => getCell(g, p) !== "X")
		).toBe(true);
		expect(
			findNetDeadCells(g).every((p) => getCell(g, p) !== "X")
		).toBe(true);
		...
		expect(isVNakadeVulnerableRegion(V)).toEqual({
			vital: { row: 4, col: 2 }
		});
		expect(isNakadeVulnerableRegion(V)).toBeNull();
		expect(isLNakadeVulnerableRegion(V)).toBeNull();
		expect(isSquareNakadeVulnerableRegion(V)).toBeNull();
		expect(isPyramidNakadeVulnerableRegion(V)).toBeNull();
		expect(isTwistedNakadeVulnerableRegion(V)).toBeNull();
		expect(isL4NakadeVulnerableRegion(V)).toBeNull();
		expect(isStraight4NakadeVulnerableRegion(V)).toBeNull();
		expect(isBulky5NakadeVulnerableRegion(V)).toBeNull();
		expect(isPlusNakadeVulnerableRegion(V)).toBeNull();
		const dead = findVNakadeDeadCells(g);
		expect(dead.length).toBe(xGroups[0]!.stones.length);
		expect(dead.every((p) => getCell(g, p) === "X")).toBe(true);
		expect(areaOutcome(g).winner).toBe("X");
		expect(
		...
		// plusNakadeDeath alone leaves X
		expect(
			areaOutcome(
				g,
				false,
		...
		const cleared = scoreArea(removeVNakadeDeadStones(g));
		expect(cleared.O).toBeGreaterThan(cleared.X);
		// Plus is not V
		expect(
			isVNakadeVulnerableRegion([
	...
	it("go-lite-v-nakade: seeded double-pass O wins; without flag X wins; contrasts; replay faithful", () => {
		const cfg = examplePresets["go-lite-v-nakade"].config;
		const { kernel, gameConfig } = compileConfig(cfg);
		const script: KernelAction[] = [{ type: "pass" }, { type: "pass" }];
		let state = kernel.initialState(cfg.rng.seed);
	...
	it("validates and compiles the go-lite-rabbity6-nakade preset", () => {
		const cfg = examplePresets["go-lite-rabbity6-nakade"].config;
		expect(validateConfig(cfg).ok).toBe(true);
		const { kernel, gameConfig } = compileConfig(cfg);
		expect(gameConfig.objectiveMode).toBe("area_control");
		expect(gameConfig.rabbitySixNakadeDeath).toBe(true);
		expect(gameConfig.captureMode).toBe("liberties");
		const state = kernel.initialState(cfg.rng.seed);
		expect(kernel.legalActions(state, 0).some((a) => a.type === "pass")).toBe(
			true
	...
	it("rejects objective.rabbitySixNakadeDeath outside area_control", () => {
		const bad = structuredClone(examplePresets["tic-tac-toe"].config) as {
			objective: { mode: string; rabbitySixNakadeDeath?: boolean };
		};
		bad.objective = { mode: "n_in_a_row", rabbitySixNakadeDeath: true };
		expect(validateConfig(bad as never).ok).toBe(false);
	});

	it("findRabbitySixNakadeDeadCells: 2×3 corridor clears Benson-alive one-eyed shell; prior nakade + chase miss", () => {
		const cfg = examplePresets["go-lite-rabbity6-nakade"].config;
		const { kernel } = compileConfig(cfg);
		const state = kernel.initialState(cfg.rng.seed);
		const g = state.grid;
		...
		expect(
			findVNakadeDeadCells(g).every((p) => getCell(g, p) !== "X")
		).toBe(true);
		expect(
			findNetDeadCells(g).every((p) => getCell(g, p) !== "X")
		).toBe(true);
		...
		expect(isRabbitySixNakadeVulnerableRegion(R6)).toEqual({
			vital: { row: 2, col: 3 }
		});
		expect(isNakadeVulnerableRegion(R6)).toBeNull();
		expect(isLNakadeVulnerableRegion(R6)).toBeNull();
		expect(isSquareNakadeVulnerableRegion(R6)).toBeNull();
		expect(isPyramidNakadeVulnerableRegion(R6)).toBeNull();
		expect(isTwistedNakadeVulnerableRegion(R6)).toBeNull();
		expect(isL4NakadeVulnerableRegion(R6)).toBeNull();
		expect(isStraight4NakadeVulnerableRegion(R6)).toBeNull();
		expect(isBulky5NakadeVulnerableRegion(R6)).toBeNull();
		expect(isPlusNakadeVulnerableRegion(R6)).toBeNull();
		expect(isVNakadeVulnerableRegion(R6)).toBeNull();
		const dead = findRabbitySixNakadeDeadCells(g);
		expect(dead.length).toBe(xGroups[0]!.stones.length);
		expect(dead.every((p) => getCell(g, p) === "X")).toBe(true);
		expect(areaOutcome(g).winner).toBe("X");
		expect(
		...
		const cleared = scoreArea(removeRabbitySixNakadeDeadStones(g));
		expect(cleared.O).toBeGreaterThan(cleared.X);
		// Bulky-5 P is not rabbity-six
		expect(
			isRabbitySixNakadeVulnerableRegion([
				{ row: 2, col: 2 },
				{ row: 2, col: 3 },
		...
		// U (2×3 minus edge-middle) is not rabbity-six
		expect(
			isRabbitySixNakadeVulnerableRegion([
				{ row: 2, col: 2 },
				{ row: 2, col: 4 },
	...
	it("go-lite-rabbity6-nakade: seeded double-pass O wins; without flag X wins; contrasts; replay faithful", () => {
		const cfg = examplePresets["go-lite-rabbity6-nakade"].config;
		const { kernel, gameConfig } = compileConfig(cfg);
		const script: KernelAction[] = [{ type: "pass" }, { type: "pass" }];
		let state = kernel.initialState(cfg.rng.seed);
	...
	it("validates and compiles the go-lite-u-nakade preset", () => {
		const cfg = examplePresets["go-lite-u-nakade"].config;
		expect(validateConfig(cfg).ok).toBe(true);
		const { kernel, gameConfig } = compileConfig(cfg);
		expect(gameConfig.objectiveMode).toBe("area_control");
		expect(gameConfig.uNakadeDeath).toBe(true);
		expect(gameConfig.captureMode).toBe("liberties");
		const state = kernel.initialState(cfg.rng.seed);
		expect(kernel.legalActions(state, 0).some((a) => a.type === "pass")).toBe(
			true
	...
	it("rejects objective.uNakadeDeath outside area_control", () => {
		const bad = structuredClone(examplePresets["tic-tac-toe"].config) as {
			objective: { mode: string; uNakadeDeath?: boolean };
		};
		bad.objective = { mode: "n_in_a_row", uNakadeDeath: true };
		expect(validateConfig(bad as never).ok).toBe(false);
	});

	it("findUNakadeDeadCells: U corridor clears Benson-alive one-eyed shell; prior nakade + chase miss", () => {
		const cfg = examplePresets["go-lite-u-nakade"].config;
		const { kernel } = compileConfig(cfg);
		const state = kernel.initialState(cfg.rng.seed);
		const g = state.grid;
		...
		expect(
			findRabbitySixNakadeDeadCells(g).every((p) => getCell(g, p) !== "X")
		).toBe(true);
		expect(
			findNetDeadCells(g).every((p) => getCell(g, p) !== "X")
		).toBe(true);
		...
		expect(isUNakadeVulnerableRegion(U)).toEqual({
			vital: { row: 3, col: 3 }
		});
		expect(isNakadeVulnerableRegion(U)).toBeNull();
		expect(isLNakadeVulnerableRegion(U)).toBeNull();
		expect(isSquareNakadeVulnerableRegion(U)).toBeNull();
		expect(isPyramidNakadeVulnerableRegion(U)).toBeNull();
		expect(isTwistedNakadeVulnerableRegion(U)).toBeNull();
		expect(isL4NakadeVulnerableRegion(U)).toBeNull();
		expect(isStraight4NakadeVulnerableRegion(U)).toBeNull();
		expect(isBulky5NakadeVulnerableRegion(U)).toBeNull();
		expect(isPlusNakadeVulnerableRegion(U)).toBeNull();
		expect(isVNakadeVulnerableRegion(U)).toBeNull();
		expect(isRabbitySixNakadeVulnerableRegion(U)).toBeNull();
		const dead = findUNakadeDeadCells(g);
		expect(dead.length).toBe(xGroups[0]!.stones.length);
		expect(dead.every((p) => getCell(g, p) === "X")).toBe(true);
		expect(areaOutcome(g).winner).toBe("X");
		expect(
		...
		// rabbitySixNakadeDeath alone leaves X
		expect(
			areaOutcome(
				g,
				false,
		...
		const cleared = scoreArea(removeUNakadeDeadStones(g));
		expect(cleared.O).toBeGreaterThan(cleared.X);
		// Bulky-5 P (corner hole) is not U
		expect(
			isUNakadeVulnerableRegion([
				{ row: 2, col: 2 },
				{ row: 2, col: 3 },
		...
		// Bottom-open U also matches; vital opposite
		expect(
			isUNakadeVulnerableRegion([
				{ row: 2, col: 2 },
				{ row: 2, col: 3 },
	...
	it("go-lite-u-nakade: seeded double-pass O wins; without flag X wins; contrasts; replay faithful", () => {
		const cfg = examplePresets["go-lite-u-nakade"].config;
		const { kernel, gameConfig } = compileConfig(cfg);
		const script: KernelAction[] = [{ type: "pass" }, { type: "pass" }];
		let state = kernel.initialState(cfg.rng.seed);
