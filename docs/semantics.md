...
- pass: consecutivePasses++; may terminal (area_control). With
  ...
  consecutive passes in marking remove marked stones then score. With
  `objective.markDeadResume`, `rejectMarks` (when marks are non-empty) clears
  `markedDead`, exits `markingPhase`, and resumes placement without scoring;
  a later double-pass may re-enter marking.
...
| jump | movement.capture = jump | Leap over adjacent enemy to empty beyond (rect rays, hex cube-axis, or graph 2-edge); mid cleared; further jumps keep seat under alternating (`mustContinueFrom`); simultaneous rectangle is single-hop by default or multi-hop via `actionsPerTurn > 1` (joint vacated-origin mid clear per index, or ordered sequential mid clear with capture-before-flee at mid; no `mustContinueFrom`); optional `mustCapture`; optional `mustLongestCapture` (max chain length; alternating only); incompatible with `graphReach: hop` |
...
- capture jump: rectangle | hex_offset | graph; quiet range 1; jump distance
  ...
  mid clear per indexed pair — joint vacated-origin or ordered sequential
  (Ordered Simultaneous Jump Race; capture-before-flee at mid;
  flee-before-jump rejects); `actionsPerTurn > 1` allows same-piece multi-hop
  rounds without mustContinueFrom (Double Simultaneous Jump Race); optional
  `mustCapture` forbids quiet moves at turn/round start when any jump exists
  (Mandatory Jump Race); optional `mustLongestCapture` (requires `mustCapture`;
...
- promotion: optional `movement.promotion` (rectangle | hex_offset | graph +
  ...
  `menForwardOnly` (rectangle | hex_offset | graph) restricts
  uncrowned quiet/jump toward the promotion side (Forward Men Jump Lite /
  Hex Forward Men Jump Lite / Graph Forward Men Jump Lite /
  Graph Hub Forward Men Jump Lite; hex filters cube-axis lands by offset-row
  sign; graph `targetRows`: strict decrease in shortest-path edge distance to
  any node on `targetRows[seat]`; graph `targetNodes`: strict decrease toward
...
liberty (after Benson/deadStones, before nakadeDeath / netDeath /
...
vital fill (after optional squareNakadeDeath, before twistedNakadeDeath /
netDeath; edge + seki exempt; distinct from square-4 / T1/L so T-shape bulky
eyes stay contrastable);
optional `objective.twistedNakadeDeath` removes interior groups bordering a
twisted-4 (Z/S) nakade big-eye that would have <2 true eyes after an opponent
vital fill (after optional pyramidNakadeDeath, before l4NakadeDeath / netDeath;
edge + seki exempt; distinct from pyramid-4 / square-4 / T1/L / L4 so skew bulky
eyes stay contrastable);
...
opponent vital fill (after optional l4NakadeDeath, before bulky5NakadeDeath /
netDeath; edge + seki exempt; distinct from L4 / twisted / pyramid / square /
T1 / bent-3 so the last free tetromino stays contrastable);
optional `objective.bulky5NakadeDeath` removes interior groups bordering a
bulky-5 (P-pentomino) nakade big-eye that would have <2 true eyes after an
opponent vital fill (after optional straight4NakadeDeath, before
plusNakadeDeath / netDeath; edge + seki exempt; distinct from tetromino nakade
tables so the first pentomino big-eye stays contrastable);
optional `objective.plusNakadeDeath` removes interior groups bordering a plus /
X-pentomino nakade big-eye that would have <2 true eyes after an opponent
vital fill (after optional bulky5NakadeDeath, before vNakadeDeath / netDeath;
edge + seki exempt; distinct from bulky-5 (P) and tetromino nakade tables so
...
uNakadeDeath / netDeath; edge + seki exempt; first 6-cell nakade class; distinct
from V / plus / bulky-5 and tetromino tables);
optional `objective.uNakadeDeath` removes interior groups bordering a
U-pentomino (2×3 / 3×2 minus edge-middle) nakade big-eye that would have <2 true
eyes after an opponent vital fill (after optional rabbitySixNakadeDeath, before
netDeath; edge + seki exempt; distinct from bulky-5 corner-hole and other
pentomino / tetromino / hexomino tables);
...
+ pass legal until two consecutive passes score; optional `objective.markDead`
  ...
  groups as dead, then two-pass removes marked stones and scores; optional
  `objective.markDeadResume` adds `rejectMarks` to exit marking without scoring
  when marks exist) |
